...
function updateProps (domNode, oldVirtualNode, oldProps, newVirtualNode, newProps) {
  if (oldProps) {
    ...
    for (var i = 0; i < oldPropsNames.length; i++) {
      var name = oldPropsNames[i]
      if (name === 'ref' || name === 'on') continue
      if (name in EVENT_LISTENER_PROPS) continue
      var oldValue = oldProps[name]
      if (HOOKS.isHook(oldValue) && oldValue.unhook) {
        oldValue.unhook(domNode, name, newProps ? newProps[name] : undefined)
      } else if (!newProps || !(name in newProps)) {
        if (name === 'dataset') {
